...
const authenticateToken = (req, res, next) => {
    ...
    jwt.verify(
        ...
        (err, user) => {
            ...
            req.user = user; // after successful decoding token, we received a payload and we are attaching that payload ID to the request object so we can access it inside our controller 
            next();
        }
    );
